/**
 * Tesbih Kuyusu V6.4.3 - Ultra Operasyon Core
 *
...
var TK6 = (function () {
  ...
  function getUltraPanelHazirlik_(phone) {
    ...
    return {
      ...
      cargo: {
        ...
        company: row["Son_Kargo_Firması"] || CFG.defaultCargoCompany
      },
      invoice: {
  ...
  function ultraSiparisPaneli_() {
    showDialog_("ultraSiparisPaneli", "Ultra sipariş paneli", 1800, 980);
  }
  ...
  function v643GercekSheetKabulKontrolu_() {
    var started = Date.now();
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    sistemKolonlariniHazirla_();
    var payloads = v643KabulPayloadlari_();
    var tekSonuclar = [];
    ...
    var ozet = v643KabulSayimOzeti_(ss);
    return {
      ...
      salesPost: "Yapılmadı",
      counts: ozet,
      status: "V6.4.3 gerçek Sheet kabul veri akışı tamamlandı"
    };
  }

  function v643KabulSayimOzeti_(ss) {
    return {
  ...
  function v643KabulPayloadlari_() {
  ...
  function ultraSiparisKontrolEt_(form) {
    ...
    (form.odemeler || []).forEach(function (p, index) {
      if (!p.odemeYapan) issues.push("Ödeme " + (index + 1) + ": Ödeme_Yapan eksik");
    ...
    (form.urunler || []).forEach(function (it, index) {
      ...
      if (!it.odemeYapan && (form.odemeler || []).length !== 1) issues.push("Ürün " + (index + 1) + ": ödeme yapan/fatura kişisi seçilmeli");
    ...
    if (!form.kargo || !form.kargo.kargoTel) issues.push("Kargo_Tel eksik");
  ...
  function faturaVeKargoOlustur_(openId) {
    ...
    kontrolMerkeziniGuncelle_();
    var control = panelKontrolOzetiForOpen_(ss, openId || "");
    if (!control.ok) throw new Error(control.summary.join(" | "));
    var finalBlocks = faturaKargoSonAsamaBlokajlari_(ss, openId || "");
    if (finalBlocks.length) throw new Error(finalBlocks.join(" | "));
    ...
    if (yes_(setting_(ss, CFG.liveParasutSendSetting, "Hayır"))) {
      invoice = [];
    ...
    var cargoResults = objects_(sheet_(ss, CFG.sheets.cargo))
      ...
      .map(function (row) { return navlungoKargoOlusturOnayli_(row[H.OPEN_ID]); });
    return { ok: true, openId: openId || "", invoice: invoice, cargo: cargoResults, control: control };
  }
  ...
  function musteriHafizaVarsayilanlari_(ss, phone) {
    ...
    return {
      cargoReceiver: row["Son_Kargo_Alıcısı"] || "",
      cargoPhone: row["Son_Kargo_Tel"] || "",
      city: row["Son_İl"] || "",
      district: row["Son_İlçe"] || "",
      address: row["Son_Adres"] || "",
      cargoCompany: row["Son_Kargo_Firması"] || ""
    };
  }
  ...
  function normalizePanelFaturalari_(form, ss) {
    ...
    return (form.odemeler || []).filter(function (p) { return p.odemeYapan; }).map(function (p) {
      var current = byName[p.odemeYapan] || {};
  ...
  return {
    ...
    v643GercekSheetKabulKontrolu: v643GercekSheetKabulKontrolu_,
    HEADERS: HEADERS,
    H: H,
    CFG: CFG
  };
...
function v643GercekSheetKabulKontrolu() { return TK6.v643GercekSheetKabulKontrolu(); }

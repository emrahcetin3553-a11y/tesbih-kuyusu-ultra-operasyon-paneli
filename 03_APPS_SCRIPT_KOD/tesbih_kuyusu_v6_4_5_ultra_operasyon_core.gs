/**
 * Tesbih Kuyusu V6.4.5 - Ultra Operasyon Core
 *
...
var TK6 = (function () {
  ...
  function v645GercekSheetKabulKontrolu_() {
    var started = Date.now();
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    sistemKolonlariniHazirla_();
    var payloads = v645KabulPayloadlari_();
    var tekSonuclar = [];
    ...
    var ozet = v645KabulSayimOzeti_(ss);
    return {
      ...
      salesPost: "Yapılmadı",
      counts: ozet,
      status: "V6.4.5 gerçek Sheet kabul veri akışı tamamlandı"
    };
  }

  function v645KabulSayimOzeti_(ss) {
    return {
  ...
  function v645KabulPayloadlari_() {
  ...
  function nameIssues_(label, value) {
    ...
    if (/^[^\s]{12,}$/.test(raw)) return [label + " birleşik yazılmış görünüyor; ad ve soyadı boşlukla ayırın"];
    return [];
  }
  ...
  return {
    ...
    v645GercekSheetKabulKontrolu: v645GercekSheetKabulKontrolu_,
    HEADERS: HEADERS,
    H: H,
    CFG: CFG
  };
...
function v645GercekSheetKabulKontrolu() { return TK6.v645GercekSheetKabulKontrolu(); }

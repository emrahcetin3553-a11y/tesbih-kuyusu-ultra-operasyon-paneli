...
var TK6 = (function () {
  ...
  function seciliSiparisiDuzenle_() {
    ...
    if (!ids.length) throw new Error("Seçili satırdan Açık_Sipariş_ID bulunamadı.");
    openUltraPanelForEdit_(ids.slice(0, 1));
  }

  function seciliSiparisleriDuzenle_() {
    var ids = selectedOpenIds_();
    if (!ids.length) throw new Error("Seçili satırlardan Açık_Sipariş_ID bulunamadı.");
  ...
  function musteriHafizasindanSil_() {
    ...
    if (!phone) throw new Error("Müşteri hafızasından pasifleştirmek için telefon bulunamadı.");
  ...
  function logicalPatchSelected_(mode) {
    ...
    var sh = ss.getActiveSheet();
    var range = sh.getActiveRange();
    if (!range) throw new Error("İşlem yapılacak satır seçili değil.");
    var ids = [];
    var changed = 0;
    for (var offset = 0; offset < range.getNumRows(); offset++) {
      var rowNum = range.getRow() + offset;
  ...
  function faturaVeKargoOlustur_(openId) {
    ...
    kontrolMerkeziniGuncelle_();
    var control = panelKontrolOzetiForOpen_(ss, openId || "");
    if (!control.ok) return { ok: false, openId: openId || "", status: "Fatura/kargo son adımı durdu", blocks: control.summary, control: control };
    var finalBlocks = faturaKargoSonAsamaBlokajlari_(ss, openId || "");
    if (finalBlocks.length) return { ok: false, openId: openId || "", status: "Fatura/kargo son adımı durdu", blocks: finalBlocks, control: control };
